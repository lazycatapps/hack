...
PROJECT_TYPE_RECORDS=(
    "key=lpk-only|display=LPK only|aliases=lpk-only,lpk|trigger=lpk-package.yml|additional=|hints="
    "key=docker-lpk|display=Docker image + LPK|aliases=docker-lpk,docker|trigger=docker-image.yml,lpk-package.yml|additional=cleanup-docker-tags.yml,reusable-docker-image.yml|hints=.github/workflows/docker-image.yml,.github/workflows/reusable-docker-image.yml,.github/workflows/includes/docker-image.reusable.yml"
)
...
prompt_for_project_type() {
    ...
    local record
    for record in "${PROJECT_TYPE_RECORDS[@]}"; do
        options+=("$(get_record_field "$record" "display")")
    done
    local choice
